...
const dynamicForm = () => {
	...
	const setElementVisibility = (options) => {
		...
		elements.forEach(element => {
			const showIfValues = element.dataset.showIfValue.split(',').map(item => item.trim());
	...
	forms.forEach(form => {
		...
		elements.forEach(element => {
			...
			const dependentElements = Array.from(document.querySelectorAll(`[data-show-if-field="${id}"]`));
